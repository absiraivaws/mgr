...
export const LoginPage: React.FC<LoginPageProps> = ({
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    const newUser: UserAccount = {
      ...
      phone: regPhone.trim(),
      createdAt: Date.now(),
      avatarColor: regRole === 'driver' ? 'cyan' : 'emerald',
    };

    // Save user locally and establish active session immediately
    const updatedUsers = [newUser, ...users];
    saveStoredUsers(updatedUsers);
    setCurrentUserSession(newUser);

    // Sync user to Supabase user_accounts table
    try {
      await syncUserAccountToSupabase(newUser);
    } catch (err) {
      console.warn('Could not sync newly registered user to Supabase:', err);
    }
