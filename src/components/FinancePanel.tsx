...
export const FinancePanel: React.FC<FinancePanelProps> = ({
  ...
  const canEdit = hasPermission(activeUser, 'canEditFinanceTransaction') || isAdmin;
  const canDelete = hasPermission(activeUser, 'canDeleteFinanceTransaction') || isAdmin;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`${t.cardBg} rounded-2xl p-5 sm:p-6 border shadow-xl`}>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-5 border-b mb-5" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
          ...
          {/* Today Expenses */}
          <div className="p-3.5 rounded-xl border bg-rose-500/10 border-rose-500/20 flex flex-col justify-between">
          ...
          {/* Today Profit / Loss */}
          <div className={`p-3.5 rounded-xl border flex flex-col justify-between ${todayMetrics.profit >= 0 ? 'bg-emerald-500/15 border-emerald-500/30' : 'bg-rose-500/15 border-rose-500/30'}`}>
            <div className="flex items-center justify-between text-xs font-semibold mb-1">
              <span className={todayMetrics.profit >= 0 ? 'text-emerald-400' : 'text-rose-400'}>Today Profit</span>
              <Sparkles className="w-3.5 h-3.5" />
            </div>
            <div className={`text-base sm:text-lg font-black font-mono ${todayMetrics.profit >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
              {formatCurrency(todayMetrics.profit, settings.currencySymbol, settings.currencyPosition)}
            </div>
          </div>

          {/* Month Income */}
          <div className="p-3.5 rounded-xl border bg-cyan-500/10 border-cyan-500/20 flex flex-col justify-between">
          ...
          {/* Month Expenses */}
          <div className="p-3.5 rounded-xl border bg-amber-500/10 border-amber-500/20 flex flex-col justify-between">
          ...
          {/* Month Profit / Loss */}
          <div className={`p-3.5 rounded-xl border flex flex-col justify-between ${monthMetrics.profit >= 0 ? 'bg-indigo-500/15 border-indigo-500/30' : 'bg-rose-500/15 border-rose-500/30'}`}>
            <div className="flex items-center justify-between text-xs font-semibold mb-1">
              <span className={monthMetrics.profit >= 0 ? 'text-indigo-300' : 'text-rose-400'}>Month Profit</span>
              <DollarSign className="w-3.5 h-3.5" />
            </div>
            <div className={`text-base sm:text-lg font-black font-mono ${monthMetrics.profit >= 0 ? 'text-indigo-300' : 'text-rose-400'}`}>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1 border-b" style={{ borderColor: 'rgba(255,255,255,0.1)' }}>
        <button
          type="button"
        ...
        <div className="space-y-4">
          ...
          <div className={`${t.cardBg} rounded-2xl p-4 border space-y-3`}>
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
              {/* Type Pills */}
              <div className="flex items-center gap-1.5 p-1 rounded-xl bg-slate-900/80 border border-slate-800 w-fit">
                <button
                  type="button"
                  onClick={() => setTxnTypeFilter('all')}
                  className={`px-3 py-1 rounded-lg text-xs font-bold transition cursor-pointer ${
                    txnTypeFilter === 'all' ? 'bg-amber-500 text-slate-950' : 'text-slate-400 hover:text-white'
                  }`}
                >
                  All ({entries.length})
                </button>
                <button
                  ...
                  className={`px-3 py-1 rounded-lg text-xs font-bold transition cursor-pointer flex items-center gap-1 ${
                    txnTypeFilter === 'income' ? 'bg-emerald-500 text-slate-950' : 'text-emerald-400 hover:text-white'
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded-lg text-xs font-bold transition cursor-pointer flex items-center gap-1 ${
                    txnTypeFilter === 'expense' ? 'bg-rose-500 text-white' : 'text-rose-400 hover:text-white'
                  }`}
                >
              ...
              <div className="flex items-center gap-3 text-xs font-mono">
                ...
                <span className="text-slate-600">|</span>
                <span className={filteredTotals.net >= 0 ? 'text-emerald-400 font-bold' : 'text-rose-400 font-bold'}>
          ...
          <div className={`${t.cardBg} rounded-2xl border overflow-hidden shadow-xl`}>
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className={`border-b ${t.divider} ${t.cardSubtleBg} text-slate-400 uppercase tracking-wider font-semibold text-[10px]`}>
                ...
                <tbody className={`divide-y ${t.divider}`}>
                  {filteredEntries.length === 0 ? (
                    ...
                    filteredEntries.map((e) => (
                      <tr key={e.id} className="hover:bg-slate-800/30 transition group">
                        ...
                        <td className="py-2.5 px-3.5 text-slate-300 font-medium whitespace-nowrap">
                          {e.category || 'General'}
                        </td>
                        <td className="py-2.5 px-3.5 text-white font-medium max-w-xs truncate" title={e.description}>
                          {e.description}
                          {e.remarks && (
                            <span className="block text-[10px] text-slate-400 font-normal italic truncate">
                              Note: {e.remarks}
                            </span>
                          )}
                        </td>
                        ...
                        <td className="py-2.5 px-3.5 text-slate-300 uppercase text-[10px] font-mono whitespace-nowrap">
                          {e.paymentMethod || 'cash'}
                        </td>
                        <td className="py-2.5 px-3.5 text-slate-300 text-[11px] whitespace-nowrap">
                          ...
                          <div className="flex items-center justify-center gap-1">
                            {canEdit && (
                              <button
                                ...
                                className="p-1 rounded-lg text-slate-400 hover:text-amber-400 hover:bg-amber-500/10 transition cursor-pointer"
        ...
        <div className="space-y-4">
          {!canViewPL ? (
            <div className={`p-8 rounded-2xl border text-center space-y-2 ${t.cardBg}`}>
              ...
              <p className="text-xs text-slate-400">You do not have permission to view Profit & Loss reports.</p>
            </div>
          ) : (
            <>
              ...
              <div className={`${t.cardBg} rounded-2xl p-4 border flex flex-col md:flex-row md:items-center justify-between gap-3`}>
                <div className="flex items-center gap-1.5 p-1 rounded-xl bg-slate-900/80 border border-slate-800 w-fit">
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-xs font-bold transition cursor-pointer ${
                      plPeriodPreset === 'today' ? 'bg-amber-500 text-slate-950 font-black' : 'text-slate-400 hover:text-white'
                    }`}
                  >
                    Today
                  </button>
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-xs font-bold transition cursor-pointer ${
                      plPeriodPreset === 'week' ? 'bg-amber-500 text-slate-950 font-black' : 'text-slate-400 hover:text-white'
                    }`}
                  >
                    This Week
                  </button>
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-xs font-bold transition cursor-pointer ${
                      plPeriodPreset === 'month' ? 'bg-amber-500 text-slate-950 font-black' : 'text-slate-400 hover:text-white'
                    }`}
                  >
                    This Month
                  </button>
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-xs font-bold transition cursor-pointer ${
                      plPeriodPreset === 'year' ? 'bg-amber-500 text-slate-950 font-black' : 'text-slate-400 hover:text-white'
                    }`}
                  >
                    This Year
                  </button>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs text-slate-400 font-semibold">Custom Period:</span>
                  <input
                    type="date"
                    value={plFromDate}
                    onChange={(e) => {
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg border border-slate-700 bg-slate-800 text-slate-300 hover:text-white transition ml-1"
                    title="Print Report"
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="p-4 rounded-2xl border bg-emerald-500/10 border-emerald-500/30">
                  <div className="text-xs font-semibold text-emerald-400 flex items-center justify-between mb-1">
                ...
                <div className="p-4 rounded-2xl border bg-rose-500/10 border-rose-500/30">
                  <div className="text-xs font-semibold text-rose-400 flex items-center justify-between mb-1">
                ...
                <div className={`p-4 rounded-2xl border ${plData.netProfit >= 0 ? 'bg-emerald-500/20 border-emerald-500/40' : 'bg-rose-500/20 border-rose-500/40'}`}>
                  <div className="text-xs font-semibold flex items-center justify-between mb-1">
                    <span className={plData.netProfit >= 0 ? 'text-emerald-400' : 'text-rose-400'}>Net Profit / (Loss)</span>
                    <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-black/30 font-bold">
                      {plData.profitMargin.toFixed(1)}% margin
                    </span>
                  </div>
                  <div className={`text-xl sm:text-2xl font-black font-mono ${plData.netProfit >= 0 ? 'text-emerald-300' : 'text-rose-300'}`}>
                ...
                <div className={`${t.cardBg} rounded-2xl border p-4 space-y-3`}>
                  ...
                  <div className="space-y-2">
                    {Object.keys(plData.incomeByCat).length === 0 ? (
                      <p className="text-xs text-slate-500 italic py-3 text-center">No income recorded in this period.</p>
                    ) : (
                ...
                <div className={`${t.cardBg} rounded-2xl border p-4 space-y-3`}>
                  ...
                  <div className="space-y-2">
                    {Object.keys(plData.expenseByCat).length === 0 ? (
                      <p className="text-xs text-slate-500 italic py-3 text-center">No expenses recorded in this period.</p>
                    ) : (
        ...
        <div className="space-y-4">
          {!canViewStatement ? (
            <div className={`p-8 rounded-2xl border text-center space-y-2 ${t.cardBg}`}>
              <Lock className="w-8 h-8 text-amber-400 mx-auto" />
              <h3 className="font-bold text-white">Access Denied</h3>
              <p className="text-xs text-slate-400">You do not have permission to view Statement of Accounts.</p>
            </div>
          ) : (
            <>
              ...
              <div className={`${t.cardBg} rounded-2xl p-4 border flex flex-col md:flex-row md:items-center justify-between gap-3`}>
                <div className="flex flex-wrap items-center gap-2 text-xs">
                  <span className="font-semibold text-slate-300">Statement Period:</span>
                  <input
                    type="date"
                    value={stmtFromDate}
                ...
                <div className="flex items-center gap-2">
                  ...
                  {canExport && (
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg border border-slate-700 bg-slate-800 text-slate-200 hover:bg-slate-700 text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer"
                    >
                      <Download className="w-3.5 h-3.5" />
                      <span>Export</span>
                    </button>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="p-3.5 rounded-xl border bg-slate-900/60 border-slate-800">
                  <span className="text-[11px] text-slate-400 font-semibold block">Opening Balance</span>
                  <span className={`text-base font-black font-mono ${statementData.openingBalance >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                    {formatCurrency(statementData.openingBalance, settings.currencySymbol, settings.currencyPosition)}
                  </span>
                </div>

                <div className="p-3.5 rounded-xl border bg-rose-500/10 border-rose-500/20">
                  <span className="text-[11px] text-rose-400 font-semibold block">Total Debit (Expense)</span>
                ...
                <div className="p-3.5 rounded-xl border bg-emerald-500/10 border-emerald-500/20">
                  <span className="text-[11px] text-emerald-400 font-semibold block">Total Credit (Income)</span>
                ...
                <div className={`p-3.5 rounded-xl border ${statementData.closingBalance >= 0 ? 'bg-indigo-500/15 border-indigo-500/30' : 'bg-rose-500/15 border-rose-500/30'}`}>
                  <span className={`text-[11px] font-semibold block ${statementData.closingBalance >= 0 ? 'text-indigo-300' : 'text-rose-400'}`}>
                    Closing Balance
                  </span>
                  <span className={`text-base font-black font-mono ${statementData.closingBalance >= 0 ? 'text-indigo-200' : 'text-rose-300'}`}>
                  ...
                  <table className="w-full text-left border-collapse text-xs">
                    <thead>
                      <tr className={`border-b ${t.divider} ${t.cardSubtleBg} text-slate-400 uppercase tracking-wider font-semibold text-[10px]`}>
                        ...
                        <th className="py-3 px-3.5">Description</th>
                        <th className="py-3 px-3.5">Category</th>
                        <th className="py-3 px-3.5 text-right text-rose-400">Debit (Expense)</th>
                        <th className="py-3 px-3.5 text-right text-emerald-400">Credit (Income)</th>
                    ...
                    <tbody className={`divide-y ${t.divider}`}>
                      {/* Opening Balance Row */}
                      <tr className="bg-slate-900/30 font-semibold text-slate-400">
                        <td className="py-2.5 px-3.5 font-mono text-[11px]">{stmtFromDate}</td>
                        <td className="py-2.5 px-3.5 font-mono text-[11px]">—</td>
                        <td className="py-2.5 px-3.5 text-slate-300 italic">Balance Brought Forward (Opening Balance)</td>
                        <td className="py-2.5 px-3.5">—</td>
                        <td className="py-2.5 px-3.5 text-right font-mono">—</td>
                        <td className="py-2.5 px-3.5 text-right font-mono">—</td>
                        <td className={`py-2.5 px-3.5 text-right font-mono font-bold ${statementData.openingBalance >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                          {formatCurrency(statementData.openingBalance, settings.currencySymbol, settings.currencyPosition)}
                        </td>
                      ...
                      {statementData.ledgerRows.map((r) => (
                        <tr key={r.id} className="hover:bg-slate-800/30 transition">
                          <td className="py-2 px-3.5 font-mono text-[11px] text-slate-300 whitespace-nowrap">{r.date}</td>
                          <td className="py-2 px-3.5 font-mono text-[11px] font-bold text-amber-400 whitespace-nowrap">
                            {r.reference || '—'}
                          </td>
                          <td className="py-2 px-3.5 text-white font-medium max-w-xs truncate" title={r.description}>
                            {r.description}
                          </td>
                          <td className="py-2 px-3.5 text-slate-300 text-[11px] whitespace-nowrap">{r.category}</td>
                          <td className="py-2 px-3.5 text-right font-mono text-rose-400 whitespace-nowrap">
                            {r.debit > 0 ? formatCurrency(r.debit, settings.currencySymbol, settings.currencyPosition) : '—'}
                          </td>
                          <td className="py-2 px-3.5 text-right font-mono text-emerald-400 whitespace-nowrap">
                          ...
                          <td className={`py-2 px-3.5 text-right font-mono font-bold whitespace-nowrap ${
                            r.runningBalance >= 0 ? 'text-slate-100' : 'text-rose-400'
                          }`}>
                            {formatCurrency(r.runningBalance, settings.currencySymbol, settings.currencyPosition)}
                          </td>
                          <td className="py-2 px-3.5 text-slate-300 text-[11px] whitespace-nowrap">{r.who || r.cashierName || 'Staff'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          <div className={`${t.cardBg} rounded-2xl p-5 border space-y-4 shadow-lg`}>
            ...
            <div className="space-y-1.5 max-h-80 overflow-y-auto pr-1">
              {categoriesConfig.incomeCategories.map((cat) => {
                ...
                return (
                  <div key={cat} className="flex items-center justify-between p-2.5 rounded-xl border border-slate-800 bg-slate-900/50 text-xs">
                    <div className="flex items-center gap-2">
                      <Tag className="w-3.5 h-3.5 text-emerald-400" />
                      <span className="font-semibold text-slate-200">{cat}</span>
                      {isProtected && (
                        <span className="text-[9px] uppercase font-mono px-1.5 py-0.2 rounded bg-emerald-500/20 text-emerald-300">
                          System Required
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] text-slate-500 font-mono">{count} entries</span>
                      {!isProtected && (
                        <button
                          type="button"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-xs animate-in fade-in">
          <div className={`w-full max-w-lg ${t.cardBg} rounded-2xl border shadow-2xl p-5 sm:p-6 space-y-4`}>
            ...
            <form onSubmit={handleSaveTransaction} className="space-y-3.5">
              ...
              {/* Action Buttons */}
              <div className="flex items-center justify-end gap-2.5 pt-3 border-t" style={{ borderColor: 'rgba(255,255,255,0.1)' }}>
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-xs animate-in fade-in">
          <div className={`w-full max-w-sm ${t.cardBg} rounded-2xl border shadow-2xl p-5 space-y-4`}>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-bold text-sm text-white">Confirm Delete Transaction</h4>
                <p className="text-xs text-slate-400">This action cannot be undone.</p>
              </div>
            </div>

            <div className="p-3 rounded-xl bg-slate-900/60 border border-slate-800 text-xs space-y-1">
              <div className="text-slate-300 font-semibold">{deletingEntry.description}</div>
            ...
            <div className="flex items-center justify-end gap-2 pt-2">
              <button
                ...
                className="px-3.5 py-1.5 rounded-xl text-xs font-semibold text-slate-400 hover:text-white transition cursor-pointer"
              >
                Cancel
              </button>
              <button
