...
export const MGROwnersDriversView: React.FC<MGROwnersDriversViewProps> = ({
  ...
  const [driverOwnerId, setDriverOwnerId] = useState(owners[0]?.id || '');
    ...
    <div className="space-y-4">
      ...
      {isAddingDriver && (() => {
        ...
        const eligibleVehiclesForDriver = vehicles.filter(v => {
          ...
          if (currentAssignedOwnerId) {
            return v.ownerId === currentAssignedOwnerId;
          }
          return true;
        });
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
            <div className="bg-white rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl border border-slate-200">
              ...
              <form onSubmit={handleSaveDriver} className="space-y-4 text-xs">
                {/* Owner Assignment (Requirement 7) */}
                <div>
                  ...
                  {!isAdmin ? (
                    ...
                    <div>
                      <select
                        value={driverOwnerId || registeredOwner?.id || owners[0]?.id || ''}
                        onChange={e => {
                      ...
                      <span className="text-[10px] text-purple-600 font-semibold mt-0.5 block">
                        Admin Portal: Can view and assign all owners and vehicles.
                      </span>
                    </div>
                  )}
                </div>

                {/* Vehicle Selection (Requirement 7) */}
                <div>
                  ...
                  <span className="text-[10px] text-slate-500 mt-1 block">
                    {!isAdmin
                      ? `Vehicle registration number clearly displayed. Showing only ${eligibleVehiclesForDriver.length} vehicle(s) belonging to your fleet.`
                      : `Admin Portal: Showing ${eligibleVehiclesForDriver.length} vehicle(s) matching the selected owner.`}
                  </span>
                </div>
