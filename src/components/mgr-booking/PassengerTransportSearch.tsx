...
import { Search, MapPin, Calendar, Clock, Users, Car, Bus, Ship, Filter, RotateCcw } from 'lucide-react';
...
export const PassengerTransportSearch: React.FC<PassengerTransportSearchProps> = ({
  ...
  const vehicleTypeButtons: { type: TransportType | 'all'; label: string; icon: React.ReactNode }[] = [
    ...
    { type: 'bus', label: 'Buses (AC Coaches)', icon: <Bus className="w-3.5 h-3.5" /> },
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-200">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Book whole cars, luxury vans, express bus seats, or coastal island boats.
          </p>
        </div>

        {/* Popular Route Shortcuts */}
        <div className="flex flex-wrap items-center gap-1.5 text-xs">
          <span className="text-[11px] text-slate-500 font-semibold mr-1">Popular Routes:</span>
          {availableRoutes.slice(0, 3).map((r, i) => (
            <button
              key={i}
              type="button"
              onClick={() => update({ fromLocation: r.from, toLocation: r.to })}
